...
export const BookmarksPage: React.FC = () => {
  ...
  const removeBookmark = (id: number) => {
    ...
    console.log('Remove bookmark for resource:', id);
  };

  const BookmarkCard = ({ resource }: { resource: BookmarkedResource }) => (
      ...
      <div className="p-6">
        ...
        <div className="space-y-2 text-sm text-gray-500 mb-4">
          ...
          <div className="flex items-center">
            ...
            <span>Bookmarked {resource.bookmarkedAt}</span>
          </div>
        </div>
  ...
  const BookmarkListItem = ({ resource }: { resource: BookmarkedResource }) => (
      ...
      {filteredResources.length > 0 ? (
        viewMode === 'grid' ? (
          ...
          <div className="space-y-4">
            {filteredResources.map((resource) => (
              <BookmarkListItem key={resource.id} resource={resource} />
            ))}
          </div>
        )
      ) : (
